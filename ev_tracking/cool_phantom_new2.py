...
def show_graph_values(FA, streamlines, seeds_nodes_graph, show_final=True):
    ...
    #starting_point = starting_point[None,:]

    #point_actor1 = fvtk.point(starting_point,colors1, point_radius=5)
    #colors2 = np.array((0,1,1))
    #goal_point = goal_point[None,:]
    #point_actor2 = fvtk.point(goal_point,colors2, point_radius=5)
    #r.add(point_actor1)
    #r.add(point_actor2)
...
#    seeds = utils.seeds_from_mask(seed_mask, density=[2, 2, 2], affine=affine)
    ...
    for i in range(len(n)):
        ...
        if norm(seed - test) < 5:
            #print(i)
            #point_actor = fvtk.point(seed[None,:],colors, point_radius=0.3)
            #r.add(point_actor)
        #else:
        #    point_actor = fvtk.point(seed[None,:],color3, point_radius=0.3)
        #    r.add(point_actor)
